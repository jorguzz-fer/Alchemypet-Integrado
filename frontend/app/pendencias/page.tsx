...
export default function PendenciasPage() {
  ...
  // Carrega lista de clinicas uma vez.
  useEffect(() => {
    const ctrl = new AbortController();
    api
      .clinicas(ctrl.signal)
    ...
    async (f: FiltrosPendencias, signal?: AbortSignal) => {
      ...
      try {
        const r = await api.listPendencias(
          { ...f, per_page: f.per_page ?? PER_PAGE },
          signal,
        );
        setResp(r);
      } catch (e) {
  ...
  async function importar(file: File) {
    ...
    setAviso(null);
    try {
      const r = await api.importar(file);
      setAviso(
  ...
  function exportarCSV() {
    ...
    const linhas = [
      ...
      ...items.map((it) =>
        cols.map((c) => csvEscape(it[c.key])).join(';'),
      ),
    ];
    ...
    a.href = url;
    a.download = `pendencias-pagina-${page}.csv`;
    a.click();
    ...
    <div className="page">
      <div className="page-head">
        <div>
          <h1>Pendências</h1>
          <div className="lead">
            Gestão operacional das pendências de convênio.
          </div>
        </div>
